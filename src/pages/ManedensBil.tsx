...
const ManedensBil = () => {
  ...
  const { data: previousCars } = useQuery({
    ...
    <Layout>
      ...
      <section className="bg-primary text-primary-foreground py-16">
        <div className="container mx-auto px-4">
          ...
          <p className="font-serif text-xl text-center max-w-2xl mx-auto opacity-90">
            Hver måned løfter vi frem en spesiell Simca fra vårt fellesskap – med sin unike historie og sjarm.
          </p>
        </div>
      </section>
          ...
          {isLoading ? (
            ...
            <div className="grid lg:grid-cols-2 gap-12 items-start">
              ...
              <div>
                ...
                {/* Tags */}
                {featuredCar.tags && featuredCar.tags.length > 0 && (
                  <div className="flex flex-wrap gap-2 mb-6">
                    {featuredCar.tags.map((tag, index) => (
                      <span
                        key={index}
                ...
                {featuredCar.story && (
                  <div className="prose prose-lg mb-8">
                    <p className="font-serif text-lg leading-relaxed">
                      {featuredCar.story.length > 500
                        ? `${featuredCar.story.substring(0, 500)}...`
                        : featuredCar.story}
                    </p>
                  </div>
                )}
                ...
                <Link
                  ...
                  className="inline-flex items-center gap-2 bg-accent text-accent-foreground px-6 py-3 font-display uppercase text-lg border-2 border-foreground shadow-brutal hover-lift"
                >
                  Les hele historien
            ...
            <div className="text-center py-16 bg-card border-4 border-foreground">
              ...
              <Link
                to="/biler"
                className="inline-flex items-center gap-2 bg-primary text-primary-foreground px-6 py-3 font-display uppercase border-2 border-foreground shadow-brutal hover-lift"
              >
                Se alle biler
                <ArrowRight className="w-5 h-5" />
              </Link>
      ...
      {/* Previous Cars */}
      {previousCars && previousCars.length > 0 && (
              ...
              {previousCars.map((car) => {
                ...
                return (
                  <Link
                    key={car.id}
                    to={`/biler/${car.slug}`}
                    className="bg-card border-4 border-foreground shadow-brutal hover-lift block"
                  >
                    {mainImage ? (
                      <img
