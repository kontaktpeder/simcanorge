...
BEGIN
  ...
  SELECT COALESCE(NULLIF(btrim(display_name), ''), 'En bruker')
  ...
  SELECT EXISTS (
    SELECT 1
    FROM public.car_owners co
    WHERE co.car_id = NEW.car_id
      AND co.role = 'owner'
  ...
  LOOP
    ...
    VALUES (
      ...
      'Ny forespørsel om redigeringstilgang',
      _requester_name || ' ber om redigeringstilgang til ' || COALESCE(_car_title, 'bilen din') || '.',
  ...
  SELECT
    ...
    CASE WHEN _car_has_any_owner
      THEN 'Ny forespørsel om redigeringstilgang'
      ELSE 'Forespørsel mangler eier'
    END,
    _requester_name || ' ber om redigeringstilgang til ' || COALESCE(_car_title, 'en bil') ||
